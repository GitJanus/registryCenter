...
var defaultFormatter = &logutil.TextFormatter{DisableColors: true}
...
type PathMap map[logutil.Level]string
...
type WriterMap map[logutil.Level]io.Writer

// LfsHook is a hook to handle writing to local log files.
type LfsHook struct {
	paths     PathMap
	writers   WriterMap
	levels    []logutil.Level
	lock      *sync.Mutex
	formatter logutil.Formatter
...
func NewHook(output interface{}, formatter logutil.Formatter) *LfsHook {
	hook := &LfsHook{
		lock: new(sync.Mutex),
	}
...
func (hook *LfsHook) SetFormatter(formatter logutil.Formatter) {
	hook.lock.Lock()
	...
	if formatter == nil {
		...
		case *logutil.TextFormatter:
			textFormatter := formatter.(*logutil.TextFormatter)
...
func (hook *LfsHook) Fire(entry *logutil.Entry) error {
	hook.lock.Lock()
	defer hook.lock.Unlock()
...
func (hook *LfsHook) ioWrite(entry *logutil.Entry) error {
	var (
		writer io.Writer
		msg    []byte
		err    error
...
func (hook *LfsHook) fileWrite(entry *logutil.Entry) error {
	var (
		fd   *os.File
		path string
		msg  []byte
...
func (hook *LfsHook) Levels() []logutil.Level {
	return logutil.AllLevels
}
