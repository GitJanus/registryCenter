...
func New(p string, options ...Option) (*RotateLogs, error) {
	...
	for _, o := range options {
		...
		case optkeyRotationSize:
			var unitSize = 1024 * 1024
			rotationSize = o.Value().(int) * unitSize
			if rotationSize < 0 {
				rotationSize = 0
			}
...
func (rl *RotateLogs) getWriter_nolock1(bailOnRotateFail, useGenerationalNames bool) (io.Writer, error) {
...
func (rl *RotateLogs) getWriter_nolock(bailOnRotateFail, useGenerationalNames bool) (io.Writer, error) {
	...
	if rl.outFh != nil {
		if rl.curBaseFn != baseFn {
			rl.generation = 1
			return rl.createLogFile(filename, bailOnRotateFail)
		}
		fs, err := rl.outFh.Stat()
		if err != nil {
			return rl.outFh, err
		}
		if fs.Size() >= int64(rl.rotationSize) {
			newName := strings.TrimSuffix(filename, ".log") + "_" + strconv.Itoa(rl.generation) + ".log"
			dirname := filepath.Dir(filename)
			if err := os.MkdirAll(dirname, 0755); err != nil {
				return nil, errors.Wrapf(err, "failed to create directory %s", dirname)
			}

			fh, err := os.OpenFile(newName, os.O_CREATE|os.O_APPEND|os.O_WRONLY, 0644)
			if err != nil {
				return nil, errors.Errorf("failed to open file %s: %s", rl.pattern, err)
			}
			if err := rl.rotate_nolock(newName); err != nil {
				err = errors.Wrap(err, "failed to rotate")
				if bailOnRotateFail {
					// Failure to rotate is a problem, but it's really not a great
					// idea to stop your application just because you couldn't rename
...
func (rl *RotateLogs) createLogFile(filename string, bailOnRotateFail bool) (io.Writer, error) {
	dirname := filepath.Dir(filename)
	if err := os.MkdirAll(dirname, 0755); err != nil {
		return nil, errors.Wrapf(err, "failed to create directory %s", dirname)
	}
